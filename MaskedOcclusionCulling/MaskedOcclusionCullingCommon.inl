...
template <typename T>
MOC_FORCE_INLINE T max(const T &a, const T &b) { return a > b ? a : b; }
template <typename T>
MOC_FORCE_INLINE T min(const T &a, const T &b) { return a < b ? a : b; }
...
template <int N>
MOC_FORCE_INLINE void VtxFetch4(__mw *v, const unsigned int *inTrisPtr, int triVtx, const float *inVtx, int numLanes)
{
...
template <>
MOC_FORCE_INLINE void VtxFetch4<0>(__mw *v, const unsigned int *inTrisPtr, int triVtx, const float *inVtx, int numLanes)
{
...
template <int N>
MOC_FORCE_INLINE void VtxFetch4(__mw *v, const unsigned short *inTrisPtr, int triVtx, const float *inVtx, int numLanes)
{
	// Fetch 4 vectors (matching 1 sse part of the SIMD register), and continue to the next
	const int ssePart = (SIMD_LANES / 4) - N;
	for (int k = 0; k < 4; k++)
...
template <>
MOC_FORCE_INLINE void VtxFetch4<0>(__mw *v, const unsigned short *inTrisPtr, int triVtx, const float *inVtx, int numLanes)
{
	// Workaround for unused parameter warning
	(void)v;
	(void)inTrisPtr;
...
class MaskedOcclusionCullingPrivate : public MaskedOcclusionCullingBase
{
public:
	static inline MaskedOcclusionCullingPrivate *CreateMaskedOcclusionCulling(pfnAlignedAlloc alignedAlloc, pfnAlignedFree alignedFree);

	static inline void DestroyMaskedOcclusionCulling(MaskedOcclusionCullingPrivate *object);
	...
	inline void GetResolution(unsigned int &width, unsigned int &height) const
	{
		width = mWidth;
		height = mHeight;
	}

	inline void ComputeBinWidthHeight(unsigned int nBinsW, unsigned int nBinsH, unsigned int &outBinWidth, unsigned int &outBinHeight)
	{
		outBinWidth = (mWidth / nBinsW) - ((mWidth / nBinsW) % TILE_WIDTH);
		outBinHeight = (mHeight / nBinsH) - ((mHeight / nBinsH) % TILE_HEIGHT);
	}

	inline void SetNearClipPlane(float nearDist)
	{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	inline void MergeBuffer(MaskedOcclusionCullingBase *BufferB)
	{
		assert(mMaskedHiZBuffer != nullptr);
	...
	MOC_FORCE_INLINE int ClipPolygon(__m128 *outVtx, __m128 *inVtx, const __m128 &plane, int n) const
	{
		__m128 p0 = inVtx[n - 1];
	...
	template <MaskedOcclusionCullingBase::ClipPlanes CLIP_PLANE, int ORTHO>
	void TestClipPlane(__mw *vtxX, __mw *vtxY, __mw *vtxW, unsigned int &straddleMask, unsigned int &triMask, MaskedOcclusionCullingBase::ClipPlanes clipPlaneMask)
	{
		straddleMask = 0;
		...
		if (!ORTHO)
		{
			for (int i = 0; i < 3; ++i)
			{
				switch (CLIP_PLANE)
				{
				case ClipPlanes::CLIP_PLANE_LEFT:
					planeDp[i] = _mmw_add_ps(vtxW[i], vtxX[i]);
					break;
				case ClipPlanes::CLIP_PLANE_RIGHT:
					planeDp[i] = _mmw_sub_ps(vtxW[i], vtxX[i]);
					break;
				case ClipPlanes::CLIP_PLANE_BOTTOM:
					planeDp[i] = _mmw_add_ps(vtxW[i], vtxY[i]);
					break;
				case ClipPlanes::CLIP_PLANE_TOP:
					planeDp[i] = _mmw_sub_ps(vtxW[i], vtxY[i]);
					break;
				case ClipPlanes::CLIP_PLANE_NEAR:
					planeDp[i] = _mmw_sub_ps(vtxW[i], _mmw_set1_ps(mNearDist));
					break;
				}
			}
		}
		else
		{
			for (int i = 0; i < 3; ++i)
			{
				...
				{
				case ClipPlanes::CLIP_PLANE_LEFT:
					planeDp[i] = _mmw_add_ps(_mmw_set1_ps(1.0f), vtxX[i]);
					break;
				case ClipPlanes::CLIP_PLANE_RIGHT:
					planeDp[i] = _mmw_sub_ps(_mmw_set1_ps(1.0f), vtxX[i]);
					break;
				case ClipPlanes::CLIP_PLANE_BOTTOM:
					planeDp[i] = _mmw_add_ps(_mmw_set1_ps(1.0f), vtxY[i]);
					break;
				case ClipPlanes::CLIP_PLANE_TOP:
					planeDp[i] = _mmw_sub_ps(_mmw_set1_ps(1.0f), vtxY[i]);
					break;
				case ClipPlanes::CLIP_PLANE_NEAR:
					planeDp[i] = _mmw_sub_ps(vtxW[i], _mmw_set1_ps(mNearDist));
					break;
	...
	template <int ORTHO>
	void ClipTriangleAndAddToBuffer(__mw *vtxX, __mw *vtxY, __mw *vtxW, __m128 *clippedTrisBuffer, int &clipWriteIdx, unsigned int &triMask, unsigned int triClipMask, ClipPlanes clipPlaneMask)
	{
		if (!triClipMask)
			return;
	...
	template <int ORTHO>
	void TransformVerts(__mw *vtxX, __mw *vtxY, __mw *vtxW, const float *modelToClipMatrix)
	{
...
#if PRECISE_COVERAGE != 0
	template <int ORTHO>
	void ProjectVertices(__mwi *ipVtxX, __mwi *ipVtxY, __mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw *vtxX, const __mw *vtxY, const __mw *vtxW)
	{
#if USE_D3D != 0
...
#else
	template <int ORTHO>
	void ProjectVertices(__mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw *vtxX, const __mw *vtxY, const __mw *vtxW)
	{
#if USE_D3D != 0
		static const int vertexOrder[] = {2, 1, 0};
#else
	...
	MOC_FORCE_INLINE void GatherVerticesFast(__mw *vtxX, __mw *vtxY, __mw *vtxW, const float *inVtx, const INDEXTYPE *inTrisPtr, int numLanes)
	{
	...
	MOC_FORCE_INLINE void ComputeBoundingBox(__mwi &bbminX, __mwi &bbminY, __mwi &bbmaxX, __mwi &bbmaxY, const __mw *vX, const __mw *vY, const ScissorRect *scissor)
	{
...
#if PRECISE_COVERAGE != 0
	MOC_FORCE_INLINE void SortVertices(__mwi *vX, __mwi *vY)
	{
	...
	MOC_FORCE_INLINE int CullBackfaces(__mwi *ipVtxX, __mwi *ipVtxY, __mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw &ccwMask, BackfaceWinding bfWinding)
	{
...
#else
	MOC_FORCE_INLINE void SortVertices(__mw *vX, __mw *vY)
	{
		// Rotate the triangle in the winding order until v0 is the vertex with lowest Y value
		for (int i = 0; i < 2; i++)
		{
	...
	MOC_FORCE_INLINE int CullBackfaces(__mw *pVtxX, __mw *pVtxY, __mw *pVtxZ, const __mw &ccwMask, BackfaceWinding bfWinding)
	{
		// Reverse vertex order if non cw faces are considered front facing (rasterizer code requires CCW order)
		if (!(bfWinding & BACKFACE_CW))
		{
	...
	MOC_FORCE_INLINE void ComputeDepthPlane(const __mw *pVtxX, const __mw *pVtxY, const __mw *pVtxZ, __mw &zPixelDx, __mw &zPixelDy) const
	{
	...
	MOC_FORCE_INLINE void UpdateTileQuick(int tileIdx, const __mwi &coverage, const __mw &zTriv)
	{
	...
	MOC_FORCE_INLINE void UpdateTileAccurate(int tileIdx, const __mwi &coverage, const __mw &zTriv)
	{
		assert(tileIdx >= 0 && tileIdx < mTilesWidth * mTilesHeight);
	...
	MOC_FORCE_INLINE int TraverseScanline(int leftOffset, int rightOffset, int tileIdx, int rightEvent, int leftEvent, const __mwi *events, const __mw &zTriMin, const __mw &zTriMax, const __mw &iz0, float zx)
	{
...
#if PRECISE_COVERAGE != 0
	MOC_FORCE_INLINE int RasterizeTriangle(unsigned int triIdx, int bbWidth, int tileRowIdx, int tileMidRowIdx, int tileEndRowIdx, const __mwi *eventStart, const __mw *slope, const __mwi *slopeTileDelta, const __mw &zTriMin, const __mw &zTriMax, __mw &z0, float zx, float zy, const __mwi *edgeY, const __mwi *absEdgeX, const __mwi *slopeSign, const __mwi *eventStartRemainder, const __mwi *slopeTileRemainder)
#else
	MOC_FORCE_INLINE int RasterizeTriangle(unsigned int triIdx, int bbWidth, int tileRowIdx, int tileMidRowIdx, int tileEndRowIdx, const __mwi *eventStart, const __mwi *slope, const __mwi *slopeTileDelta, const __mw &zTriMin, const __mw &zTriMax, __mw &z0, float zx, float zy)
#endif
	{
		if (TEST_Z)
...
#define UPDATE_TILE_EVENTS_Y(i) triEvent[i] = _mmw_add_epi32(triEvent[i], triSlopeTileDelta[i]);
...
#if PRECISE_COVERAGE != 0
	MOC_FORCE_INLINE int RasterizeTriangleBatch(__mwi ipVtxX[3], __mwi ipVtxY[3], __mw pVtxX[3], __mw pVtxY[3], __mw pVtxZ[3], unsigned int triMask, const ScissorRect *scissor)
#else
	MOC_FORCE_INLINE int RasterizeTriangleBatch(__mw pVtxX[3], __mw pVtxY[3], __mw pVtxZ[3], unsigned int triMask, const ScissorRect *scissor)
#endif
	{
	...
	MOC_FORCE_INLINE CullingResult RenderTriangles(const float *inVtx, const INDEXTYPE *inTris, int nTris, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
		assert(mMaskedHiZBuffer != nullptr);
	...
	inline CullingResult RenderTriangles(const float *inVtx, const unsigned int *inTris, int nTris, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
	...
	inline CullingResult RenderTriangles(const float *inVtx, const unsigned short *inTris, int nTris, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
		CullingResult retVal;
	...
	inline CullingResult TestTriangles(const float *inVtx, const unsigned int *inTris, int nTris, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
		CullingResult retVal;
	...
	inline CullingResult TestRect(float xmin, float ymin, float xmax, float ymax, float wmin) const
	{
	...
	MOC_FORCE_INLINE void BinTriangles(const float *inVtx, const unsigned int *inTris, int nTris, TriList *triLists, unsigned int nBinsW, unsigned int nBinsH, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
		assert(mMaskedHiZBuffer != nullptr);
	...
	inline void BinTriangles(const float *inVtx, const unsigned int *inTris, int nTris, TriList *triLists, unsigned int nBinsW, unsigned int nBinsH, const float *modelToClipMatrix, BackfaceWinding bfWinding, ClipPlanes clipPlaneMask, const VertexLayout &vtxLayout)
	{
		if (vtxLayout.mStride == 16 && vtxLayout.mOffsetY == 4 && vtxLayout.mOffsetW == 12)
	...
	inline void RenderTrilist(const TriList &triList, const ScissorRect *scissor)
	{
		assert(mMaskedHiZBuffer != nullptr);
	...
	inline void ComputePixelDepthBuffer(float *depthData, bool flipY)
	{
		assert(mMaskedHiZBuffer != nullptr);
...
#if ENABLE_STATS
	inline OcclusionCullingStatistics GetStatistics()
	{
		return mStats;
	}
#endif

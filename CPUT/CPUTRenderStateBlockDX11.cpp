...
void CPUTRenderStateBlockDX11::ReadValue(CPUTConfigEntry *pValue, const CPUTRenderStateMapEntry *pRenderStateList, void *pDest)
{
    ...
    for (CPUTRenderStateMapEntry const *pCur = pRenderStateList; pCur->name.compare(_L("")); pCur++)
    {
        ...
        if (found)
        {
            ...
            switch (pCur->type)
            {
            case ePARAM_TYPE_TYPELESS:
                ASSERT(0, _L(""));
                break; // Should not get here.
            case ePARAM_TYPE_INT:
                *(int *)&((char *)pDest)[pCur->offset] = pValue->ValueAsInt();
                break;
            case ePARAM_TYPE_UINT:
                *(UINT *)&((char *)pDest)[pCur->offset] = pValue->ValueAsUint();
                break;
            case ePARAM_TYPE_BOOL:
                *(bool *)&((char *)pDest)[pCur->offset] = pValue->ValueAsBool();
                break;
            case ePARAM_TYPE_FLOAT:
                *(float *)&((char *)pDest)[pCur->offset] = pValue->ValueAsFloat();
                break;
            case ePARAM_TYPE_SHORT:
                *(short *)&((char *)pDest)[pCur->offset] = pValue->ValueAsInt();
                break;
            case ePARAM_TYPE_CHAR:
                ((char *)pDest)[pCur->offset] = pValue->ValueAsInt();
                break;
            case ePARAM_TYPE_UCHAR:
                ((UCHAR *)pDest)[pCur->offset] = pValue->ValueAsUint();
                break;
            case ePARAM_TYPE_STRING:
                break; // Not sure how to deal with this yet.  Strings can have different sizes
                // The following types must be converted from string to enum.  They achieve this with a translation map
            case ePARAM_TYPE_D3D11_BLEND:
                found = pBlendMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_BLEND_OP:
                found = pBlendOpMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_DEPTH_WRITE_MASK:
                found = pDepthWriteMaskMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_STENCIL_OP:
                found = pStencilOpMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_FILL_MODE:
                found = pFillModeMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_CULL_MODE:
                found = pCullModeMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_FILTER:
                found = pFilterMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_COMPARISON_FUNC:
                found = pComparisonMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            case ePARAM_TYPE_D3D11_TEXTURE_ADDRESS_MODE:
                found = pTextureAddressMap->FindMapEntryByName((int *)&((char *)pDest)[pCur->offset], pValue->ValueAsString());
                break;
            }

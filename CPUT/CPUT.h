...
#define DEBUGMESSAGEBOX(Title, Text)                                  \
    {                                                                 \
        CPUTOSServices::GetOSServices()->OpenMessageBox(Title, Text); \
    }
#define ASSERT(Condition, Message)                                         \
    {                                                                      \
        if (!(Condition))                                                  \
        {                                                                  \
            cString msg = cString(_T(__FUNCTION__)) + _L(":  ") + Message; \
...
public:
    ...
    virtual CPUTEventHandledCode CPUTHandleKeyboardEvent(CPUTKey key)
    {
        UNREFERENCED_PARAMETER(key);
        return CPUT_EVENT_UNHANDLED;
    }
    virtual CPUTEventHandledCode CPUTHandleMouseEvent(int x, int y, int wheel, CPUTMouseState state)
    {

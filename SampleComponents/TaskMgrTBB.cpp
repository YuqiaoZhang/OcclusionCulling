...
public:
    GenericTask()
        : mpFunc(0), mpvArg(0), muIdx(0), muSize(0), mpszSetName(NULL), mhTaskSet(TASKSETHANDLE_INVALID){};
    ...
    GenericTask(
        ...
        CHAR *pszSetName,
        TASKSETHANDLE hSet)
        : mpFunc(pFunc), mpvArg(pvArg), muIdx(uIdx), muSize(uSize), mpszSetName(pszSetName), mhTaskSet(hSet){};
...
public:
    TaskSetTbb()
        : mpFunc(NULL), mpvArg(0), muSize(0), mhTaskset(TASKSETHANDLE_INVALID), mbHasBeenWaitedOn(FALSE)
    {
        mszSetName[0] = 0;
...
TaskMgrTbb::TaskMgrTbb()
    : mpTbbContextId(NULL), mpTbbInit(NULL), miDemoModeThreadCountOverride(task_scheduler_init::automatic)
{
    memset(
        mSets,
        0x0,
...
VOID TaskMgrTbb::ReleaseHandles(
    TASKSETHANDLE *phSet,
    UINT uSet)
{
...
VOID TaskMgrTbb::CompleteTaskSet(
    TASKSETHANDLE hSet)
{

...
AABBoxMaskedRasterizerAVXMT::AABBoxMaskedRasterizerAVXMT(MaskedOcclusionCulling *moc, DepthBufferMaskedRasterizerAVXMT *rasterizer)
    : AABBoxRasterizerAVX()
{
...
void AABBoxMaskedRasterizerAVXMT::TransformAABBoxAndDepthTest(UINT taskId, UINT taskCount, UINT idx)
{
    ...
    {
        UINT end = std::min(base + kChunkSize, mNumModels);
        if (mEnableFCulling)
        {
        ...
        for (UINT i = base; i < end; i++)
        {
            ...
            if (mpInsideFrustum[idx][i] && !mpTransformedAABBox[i].IsTooSmall(setup, cumulativeMatrix))
            {
                ...
                PreTestResult res = mpTransformedAABBox[i].TransformAndPreTestAABBox(cumulativeMatrix, mMaskedOcclusionCulling);
                // if(res == ePT_UNSURE)
                //{
